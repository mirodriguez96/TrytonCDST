...
class Permissions(ModelSQL, ModelView):
    ...
    super_user = fields.Boolean('Permission_Asing',
                            states={
            'readonly': Not(Bool(Eval('user')))}, depends=['user'])

    user_permission = fields.Many2One('res.user', 'User', required=True, 
                                      states={
            'readonly': Not(Bool(Eval('super_user')))}, depends=['super_user'])

    action = fields.Many2Many('res.user-ir.action.wizard', 'user_permission', 'wizard'  ,'Permissions', 
                                      states={
            'readonly': Not(Bool(Eval('super_user')))}, depends=['super_user'])
            ...
            if len(permission) > 1:
                for valid in permission:
                    raise DuplicateUserPermission(
                        gettext('conector.permissions.msg_duplicate_user', permission=valid.user))          
...
class LinePermissions(ModelSQL):
    ...
    user_permission = fields.Many2One('conector.permissions', 'User permission',
